#===============================================================================
# Description: Generate certificates to bootstrap CockroachDB secured cluster.
...
func signCSR(cacert *x509.Certificate, ca_prikey *rsa.PrivateKey, certReq *x509.CertificateRequest, san string) []byte  {
  ...
  expiration := time.Hour * 24 * 10
    ...
    DNSNames:           []string{san,"localhost", "172.17.0.3"},
    IPAddresses:     []net.IP{net.IPv4(127, 0, 0, 1), net.IPv4(172, 17, 0, 3)},
	}
...
func main() {
  ...
  nodecert_bytes := signCSR(cacert, ca_prikey, certReq, "node")
  ...
  //==================================================
  // Create a CA signed client cert.
  //==================================================

  client_prikey, _, err := generateRSAKeyPair()

  if err != nil {
    log.Fatal(err)
  } 

  clientkey_file, err := os.Create("client.root.key")
  if err != nil {
    log.Fatal(err)
  }

  clientkey_str := exportPrivateKeyAsPem(client_prikey)
  clientkey_file.WriteString(clientkey_str)
  clientkey_file.Close()

  clientcsr_byte :=createCSRCert(client_prikey)
  clientcertReq, err := x509.ParseCertificateRequest(clientcsr_byte)

  if err != nil {
    log.Fatal(err)
  }

  clientcert_bytes := signCSR(cacert, ca_prikey, clientcertReq, "root")

  clientcert_file, err := os.Create("client.root.crt")
  if err != nil {
    log.Fatal(err)
  }

  pem.Encode(clientcert_file, &pem.Block{Type: "CERTIFICATE", Bytes: clientcert_bytes})
  clientcert_file.Close()
}
